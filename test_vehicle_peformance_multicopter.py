    ...
    for row_id in range(start_indx, end_indx):
        try:
            ...
            if eVTOL_type == 'multicopter':
                ...
                # Plot vehicle 
                # plot_vehicle(configs.cruise, save_figure = True, plot_control_points = True)
...
def mission_setup(analyses,vehicle,tj,profile_id):
    ...
    #     segment.air_speed                                  = 1.0 * Units['mph'] #speed * Units['m/s'] #110.   * Units['mph']
    #     segment.distance                                   = .5 * Units.miles #total_t * speed * Units['m'] #50.    * Units.miles   
